...
func (b *BaseController) StudentLogin(l sysReq.LoginDTO, c *gin.Context) {
	...
	if !b.studentService.CheckStudentByUserName(l.UserName) {
	...
	u := &model.Student{UserName: l.UserName, Password: l.Password}
	// if !userExist.Ldap {
	if user, err := b.studentService.Login(u); err != nil {
		//Update Counter in login Attempt
		if _, block := b.UserUseCase.UpdateCounterUserBlock(&loginAttObj, c); !block {
			return
		}
